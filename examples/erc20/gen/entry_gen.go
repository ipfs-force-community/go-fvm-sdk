...
func gen_entry(stateT reflect.Type, output string) error {
	entryMeta, err := getEntryPackageMeta(stateT)
	if err != nil {
		return err
	}

	render, err := template.New("gen entry").Parse(tml)
	if err != nil {
		return err
	}
...
func getEntryPackageMeta(stateT reflect.Type) (*entryMeta, error) {
	...
	stateName := typeName("main", stateT)
	var methodsMap []methodMap
	var imports []typegen.Import
	imports = append(imports, typegen.ImportsForType("main", stateT)...)
	...
	for _, sortedFunc := range sortedFuncs {
		...
		if functionT.NumIn() == 1 {
			...
			method.ParamsTypeName = typeName("main", functionT.In(0).Elem())
			imports = append(imports, typegen.ImportsForType("main", functionT.In(0).Elem())...)
		}
		...
		if functionT.NumOut() == 2 {
			...
			method.ReturnTypeName = typeName("main", functionT.Out(0).Elem())
			imports = append(imports, typegen.ImportsForType("main", functionT.Out(0).Elem())...)
