...
func Invoke(blockId uint32) uint32 {
	...
	case 0x2b47892d: //726108461  function name:GetBalance  alias name:

		// no params but return value/error
		state := new(contract.State)
		sdk.LoadState(ctx, state)
		callResult, err = state.GetBalance(ctx)

	case 0x32e7cdb6: //854052278  function name:Withdraw  alias name:
		...
		state := new(contract.State)
		sdk.LoadState(ctx, state)
		if err = state.Withdraw(ctx, &req); err == nil {
			callResult = typegen.CborBool(true)
		}
