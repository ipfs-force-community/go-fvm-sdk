...
func AsArray(s Store, r cid.Cid, bitwidth int) (*Array, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to root: %w", err)
	}
...
// AppendContinuous appends a value to the end of the array. Assumes continuous array.
...
func (a *Array) AppendContinuous(value cbor.Marshaler) error {
	if err := a.root.Set(a.store.Context(), a.root.Len(), value); err != nil {
		return fmt.Errorf("append failed to set index %v value %v in root %v: %w", a.root.Len(), value, a.root, err)
	}
	return nil
}
...
func (a *Array) Set(i uint64, value cbor.Marshaler) error {
	if err := a.root.Set(a.store.Context(), i, value); err != nil {
		return fmt.Errorf("failed to set index %v value %v in root %v: %w", i, value, a.root, err)
	}
	return nil
}

// TryDelete removes the value at index `i` from the AMT, if it exists.
...
func (a *Array) TryDelete(i uint64) (bool, error) {
	if found, err := a.root.Delete(a.store.Context(), i); err != nil {
		return false, fmt.Errorf("array delete failed to delete index %v in root %v: %w", i, a.root, err)
	} else { //nolint
		return found, nil
	}
}
...
func (a *Array) Delete(i uint64) error {
	if found, err := a.root.Delete(a.store.Context(), i); err != nil {
		return fmt.Errorf("failed to delete index %v in root %v: %w", i, a.root, err)
	} else if !found {
		return fmt.Errorf("no such index %v in root %v to delete: %w", i, a.root, err)
	}
	return nil
}
...
func (a *Array) BatchDelete(ix []uint64, strict bool) error {
	if _, err := a.root.BatchDelete(a.store.Context(), ix, strict); err != nil {
		return fmt.Errorf("failed to batch delete keys %v: %w", ix, err)
	}
	return nil
}

// ForEach iterates all entries in the array, deserializing each value in turn into `out` and then calling a function.
...
// Pop retrieves an array value into the 'out' unmarshaler (if non-nil), and removes the entry.
...
func (a *Array) Pop(k uint64, out cbor.Unmarshaler) (bool, error) {
	...
	if found, err := a.root.Delete(a.store.Context(), k); err != nil {
		return false, fmt.Errorf("failed to delete index %v in root %v: %w", k, a.root, err)
	} else if !found {
		return false, fmt.Errorf("can't find index %v to delete in root %v", k, a.root)
	}
	return true, nil
}
