...
func (t *Receipt) MarshalCBOR(w io.Writer) error {
	...
	// t.ExitCode (ferrors.ExitCode) (uint64)

	if err := cw.WriteMajorTypeHeader(cbg.MajUnsignedInt, uint64(t.ExitCode)); err != nil {
		return err
	}

	// t.ReturnData (types.RawBytes) (slice)
	if len(t.ReturnData) > cbg.ByteArrayMaxLen {
		return xerrors.Errorf("Byte array in field t.ReturnData was too long")
	}

	if err := cw.WriteMajorTypeHeader(cbg.MajByteString, uint64(len(t.ReturnData))); err != nil {
		return err
	}

	if _, err := cw.Write(t.ReturnData[:]); err != nil {
		return err
	}
...
func (t *Receipt) UnmarshalCBOR(r io.Reader) (err error) {
	*t = Receipt{}
...

	{
		...
		t.ExitCode = ferrors.ExitCode(extra)
	...
	if extra > cbg.ByteArrayMaxLen {
		return fmt.Errorf("t.ReturnData: byte array too large (%d)", extra)
	}
	if maj != cbg.MajByteString {
		return fmt.Errorf("expected byte array")
	}
	...
	if extra > 0 {
		t.ReturnData = make([]uint8, extra)
	}

	if _, err := io.ReadFull(cr, t.ReturnData[:]); err != nil {
		return err
	}
	// t.GasUsed (int64) (int64)
	{
