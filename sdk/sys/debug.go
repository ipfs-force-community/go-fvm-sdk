...
func Enabled(_ context.Context) (bool, error) {
	...
	if code != 0 {
		return false, ferrors.NewFvmErrorNumber(ferrors.ErrorNumber(code), "failed to get debug-enabled")
	}
...
func Log(_ context.Context, msg string) error {
	...
	if code != 0 {
		return ferrors.NewFvmErrorNumber(ferrors.ErrorNumber(code), "failed to record debug log")
	}
