...
func VerifySignatureExpect(
	...
	call := SimulatedInstance.EXPECT().VerifySignature(signature, signer, plaintext).Return(out...)
	initcall(call, op)
}

func HashBlake2bExpect(data []byte, out []interface{}, op *ExpectOptions) {

	call := SimulatedInstance.EXPECT().HashBlake2b(data).Return(out...)
	initcall(call, op)
}
func ComputeUnsealedSectorCidExpect(
	proofType abi.RegisteredSealProof,
	pieces []abi.PieceInfo, out []interface{}, op *ExpectOptions) {
	call := SimulatedInstance.EXPECT().ComputeUnsealedSectorCid(proofType, pieces).Return(out...)
	initcall(call, op)
}

func VerifySealExpect(info *proof.SealVerifyInfo, out []interface{}, op *ExpectOptions) {
	call := SimulatedInstance.EXPECT().VerifySeal(info).Return(out...)
	initcall(call, op)
}

func VerifyPostExpect(info *proof.WindowPoStVerifyInfo, out []interface{}, op *ExpectOptions) {
	call := SimulatedInstance.EXPECT().VerifyPost(info).Return(out...)
	initcall(call, op)
}
